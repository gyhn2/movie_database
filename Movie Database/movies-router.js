...
router.get("/:id", eachmovie);

//post request on individual movie page
router.post("/:id", postsmovies);
...
//GET request to an individual movie page
function eachmovie(req, res, next) {
    ...
    if (movies.hasOwnProperty(req.params.id)) {
        ...
        if (!req.session.loggedin) {
            ...
            //whether in watched list or not
        ...
        let gen = Object.values(movies).filter(x=>{return x.Genre.includes(afilm.Genre[0])})
        let sim = gen.sort(()=>0.5-Math.random());
        let index = sim.indexOf(afilm);
        if (index>-1) {
            sim.slice(index, 1);
        }
        sim = sim.slice(0,8);
        ...
        //render
        res.status(200).send(pug.renderFile("./views/movie.pug", { movies, reviews: req.app.locals.reviews, film: afilm, people, users, disable, alreadywatched, username: req.session.username, sim }));
    } else {
        let errorcode = "404";
...
//handles post requests on movie page
function postsmovies(req, res, next) {
