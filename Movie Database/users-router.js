...
//Profile or User page post requests
...
function getuser(req, res, next) {
    ...
    if (users.hasOwnProperty(req.params.id)) {
        ...
        if (req.session.loggedin && req.session.username===users[req.params.id].Username) {
            ...
            if (users[req.session.username].WatchedMovies.length===0) {
                rec = Object.values(movies).sort(()=>0.5-Math.random()).slice(0,5);
            } else {
                let gen = Object.values(movies).filter(x=>{return x.Genre.includes(movies[users[req.session.username].WatchedMovies[0]].Genre[0])})
                rec = gen.sort(()=>0.5-Math.random());
                rec = rec.filter(x=> {
                    for (let i=0; i<users[req.session.username].WatchedMovies.length; i++) {
                        if (x.Id!==users[req.session.username].WatchedMovies[i]) {
                            return x;
                        }
                    }
                });
            ...
            res.status(200).send(pug.renderFile("./views/profile.pug", {user: users[req.session.username], movies, people, users, reviews, rec}));
        } else {
